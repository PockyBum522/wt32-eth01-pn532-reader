...
void CheckForNfcTag()
{
    ...
    boolean success = m_nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, &uid[0], &uidLength, 2);    // Wait for an ISO14443A type cards (Mifare, etc.).  When one is found 'uid' will be populated with the UID, and uidLength will indicate if the uid is 4 bytes (Mifare Classic) or 7 bytes (Mifare Ultralight)
    ...
    if (success)
    {
        ...
        Serial.println("Success was true");
    }
}
